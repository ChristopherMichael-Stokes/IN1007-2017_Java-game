...
public class PreGame implements Runnable {
    ...
    private static final ImageIcon MOUSE
            = new ImageIcon("data/mouse.png");

    private static final ImageIcon WASD
            = new ImageIcon("data/wasd.png");
    ...
    /**
     * initializes all the buttons and text field, then sets the screen state to
     * the home screen
     *
    ...
    /**
     * allows the grid bag constraints to be easily changed with few lines of
     * code
     *
     ...
     * @param insets an Insets object, used to pad the external of a given
     * component
     */
...
//                panel.add(back,gbc);
            ...
            case SETTINGS:
                ...
                JLabel text = new JLabel("Type a resolution in the text boxes, "
                        + "and click apply resolution to change"
                ...
                setResolution.addActionListener((ActionEvent ae) -> {
                    ...
                    if (tempX > 1920 || tempY > 1080
                            || tempX < 800 || tempY < 600) {
                        panel.removeAll();
                        JOptionPane.showMessageDialog(panel,
                                String.valueOf(tempX) + " x "
                                + String.valueOf(tempY)
                                + " is not a supported resolution");
                    } else {
                        JOptionPane.showMessageDialog(panel, "Warning, a non 4:3"
                                + " resolution may result in visual errors");
            ...
            case HOW_TO_PLAY:
                ...
                JLabel mouseIcon = new JLabel(new ImageIcon(
                        MOUSE.getImage().getScaledInstance(
                                (int) (MOUSE.getIconWidth() * ((float) scaleX / MOUSE.getIconWidth() / 2)),
                                (int) (MOUSE.getIconHeight() * ((float) scaleY / MOUSE.getIconHeight() / 2)),
                                Image.SCALE_DEFAULT)));
                JLabel keyboardIcon = new JLabel(new ImageIcon(
                        WASD.getImage().getScaledInstance(
                                (int) (WASD.getIconWidth() * ((float) scaleX / WASD.getIconWidth() / 2)),
                                (int) (WASD.getIconHeight() * ((float) scaleY / WASD.getIconHeight() / 2)),
                                Image.SCALE_DEFAULT)));
                ...
                JTextArea hint3 = new JTextArea("As long as you have shots, you can "
                        + "shoot a projectile by clicking on the screen with "
                        + "the mouse."
                        + "  The projectile will shoot from the player towards the "
                        + "mouse.");
